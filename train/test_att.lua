...
local Test_att = torch.class("Test_att")
...
function Test_att:__init(data,model,loss,config)
...
function Test_att:run_wb_3d_att(logfunc)
   ...
   for batch,labels,n in self.data:iterator_wb_3d_att() do
      self.batch = self.batch or batch:contiguous():type(self.model:type())
      self.labels = self.labels or labels:type(self.model:type())
      self.batch:copy(batch:contiguous())
