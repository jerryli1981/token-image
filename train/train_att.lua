...
local Train_att = torch.class("Train_att")
...
function Train_att:__init(data,model,loss,config)
   -- Store the objects
   self.data = data
   self.model = model

   self.loss = nn.ParallelCriterion(true)
      :add(nn.ModuleCriterion(nn.ClassNLLCriterion(), nil, nn.Convert())) -- BACKPROP
      :add(nn.ModuleCriterion(nn.VRClassReward(model.sequential, 1), nil, nn.Convert()))
...
function Train_att:run_wb_3d_att(epoches,logfunc)
...
function Train_att:batchStep_wb_3d_att()

   self.clock = sys.clock()
   -- Get a batch of data
   self.batch_untyped,self.labels_untyped = self.data:getBatch_wb_3d_att(self.batch_untyped,self.labels_untyped)
   ...
   self.max, self.decision = self.output[1]:double():max(2)
